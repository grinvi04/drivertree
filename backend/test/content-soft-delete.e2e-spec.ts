...
describe('Content 소프트삭제 (e2e)', () => {
  ...
  it('soft-delete 후 단건·슬러그·목록·임베딩검색에서 제외되고 임베딩 이력은 보존', async () => {
    const created = await content.create({
      title: '세차 방법 완전정복',
      slug: 'sd-car-wash',
      content: '셀프 세차장에서는 고압수로 먼저 흙먼지를 제거한 뒤 폼건으로 거품을 도포합니다.',
      category: 'management',
      tags: ['세차', '관리'],
    })
    ...
    await expect(content.findOne(created.id)).rejects.toBeInstanceOf(NotFoundException)
    await expect(content.findOneBySlug('sd-car-wash')).rejects.toBeInstanceOf(NotFoundException)
  ...
  it('soft-delete 된 콘텐츠는 챗 로컬 폴백 출처에서 제외', async () => {
    ...
    const created = await content.create({
      ...
      content: '비보호 좌회전은 녹색 신호에 맞은편 직진 차량이 없을 때만 가능합니다.',
      category: 'rules',
      tags: ['좌회전'],
    })
