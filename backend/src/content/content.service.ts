...
import { CreateContentDto, UpdateContentDto, ContentQueryDto } from './content.dto'
...
export class ContentService implements OnModuleInit {
  ...
  private chunkText(text: string, maxChars = RAG_CONFIG.MAX_CHUNK_SIZE): string[] {
  ...
  async create(dto: CreateContentDto) {
    ...
    if (existing) throw new ConflictException('이미 존재하는 슬러그(Slug)입니다.')
  ...
  async findOne(id: string) {
    ...
    if (!item || item.deletedAt) throw new NotFoundException('콘텐츠를 찾을 수 없습니다.')
