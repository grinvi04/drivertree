...
describe('AllExceptionsFilter', () => {
  ...
  it('maps non-HttpException with statusCode=413 (PayloadTooLarge) to 413, not 500', () => {
    ...
    expect(jsonFn).toHaveBeenCalledWith(expect.objectContaining({ code: 'PAYLOAD_TOO_LARGE' }))
  })
  ...
  it('attaches SCREAMING_SNAKE code for 5xx server errors', () => {
    filter.catch(new Error('boom'), host)
    expect(jsonFn).toHaveBeenCalledWith(expect.objectContaining({ code: 'INTERNAL_SERVER_ERROR' }))
  })
  ...
  it('maps 429 HttpException to TOO_MANY_REQUESTS code', () => {
    ...
    expect(jsonFn).toHaveBeenCalledWith(expect.objectContaining({ code: 'TOO_MANY_REQUESTS' }))
  })
