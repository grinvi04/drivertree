...
function makeJwtStub() {
  return {
    ...
    verify: jest.fn().mockReturnValue({ sub: adminStub.id, username: adminStub.username }),
...
describe('AuthService', () => {
  ...
  describe('login()', () => {
    ...
    it('throws UnauthorizedException when admin not found', async () => {
      ...
      await expect(service.login({ username: 'nobody', password: 'pw' })).rejects.toBeInstanceOf(
  ...
  describe('refreshAccessToken()', () => {
    ...
    it('throws when refresh token JWT verification fails', async () => {
      ...
      await expect(service.refreshAccessToken('expired_token')).rejects.toBeInstanceOf(
        UnauthorizedException,
      )
    })
    ...
    it('throws when admin has no refreshTokenHash (logged out)', async () => {
      ...
      await expect(service.refreshAccessToken('some_token')).rejects.toBeInstanceOf(
        UnauthorizedException,
      )
    })

    it('throws when refresh token does not match stored hash (token reuse attack)', async () => {
      await expect(service.refreshAccessToken('tampered_token')).rejects.toBeInstanceOf(
        UnauthorizedException,
      )
    })
  })
