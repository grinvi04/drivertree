...
export class ChatService {
  ...
  private async searchSemanticChunks(
    vector: number[],
    limit = RAG_CONFIG.CHUNK_SEARCH_LIMIT,
    ...
    return rawChunks.filter((chunk) => chunk.distance <= RAG_CONFIG.SIMILARITY_THRESHOLD)
  }
  ...
  async ask(dto: AskChatDto): Promise<ChatResponseDto> {
    ...
    const vector = await this.gemini.getEmbedding(message)

    if (vector && this.gemini.isAvailable) {
      // 1. 실제 RAG 모드 (Gemini + pgvector)
      const matchedChunks = await this.searchSemanticChunks(vector)
...
${message}`
        ...
        botResponse = await this.gemini.generateContent(systemPrompt)
      } catch (error) {
        this.logger.error(
    ...
    // 대화 이력 저장
    // JSON 직렬화 후 재파싱으로 Prisma.InputJsonValue 호환 타입으로 변환
    const log = await this.prisma.chatLog.create({
      data: {
        sessionKey,
        userMessage: message,
        botResponse,
        matchedSources: JSON.parse(JSON.stringify(matchedSources)) as Prisma.InputJsonValue,
      },
    })
   ...
   */
  private async localNlpFallback(message: string, sessionKey: string): Promise<ChatResponseDto> {
    ...
    if (ranked.length > 0) {
      ...
      const cleanContent = topMatch.content.replace(/[#*`_-]/g, '').substring(0, 200) + '...'
