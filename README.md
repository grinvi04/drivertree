...
NestJS + Next.js + PostgreSQL(pgvector) + Gemini(RAG) 풀스택 서비스. 자세한 기획은 [`PRD.md`](./PRD.md) 참고.
...
| RAG | Gemini `text-embedding-004` (768-dim) → pgvector 코사인 유사도 검색 → `gemini-1.5-flash`. **API Key 미설정 시** 로컬 TF-IDF + 코사인 폴백으로 자동 전환. |
...
│     ├─ content/        가이드 CRUD + 임베딩 인덱싱 + 캐시
│     ├─ chat/           RAG 챗봇 + 로컬 NLP 폴백
│     ├─ calculator/     범칙금 / 유지비 계산기
│     └─ common/         Exception Filter · Request Logger · 로컬 NLP 헬퍼
...
```bash
cd backend
cp .env.example .env          # DATABASE_URL · JWT_SECRET · JWT_REFRESH_SECRET · GEMINI_API_KEY
npm install
npx prisma generate
...
- 가이드 6건 시드 + Gemini 임베딩 적재 (API Key 없으면 텍스트만 적재)
...
`GEMINI_API_KEY`가 없거나 호출 실패 시 `common/local-nlp.helper.ts`의 TF-IDF + 코사인 폴백이 자동 가동.
