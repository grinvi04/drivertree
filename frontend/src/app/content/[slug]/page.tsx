...
async function getRelated(category: string, excludeId: string): Promise<GuideContent[]> {
  try {
...
const CALLOUT_META: Record<
  string,
  { icon: string; label: string; bg: string; border: string; labelColor: string }
> = {
...
function renderCallout(type: string, contentLines: string[]): string {
  ...
  for (const line of contentLines) {
    if (/^[-*]\s/.test(line)) {
      if (!inList) {
        inner.push('<ul style="margin:8px 0 4px 18px;padding:0;list-style:disc">')
        inList = true
      }
      inner.push(
        `<li style="font-size:15px;line-height:1.65;color:#1d1d1f;margin-bottom:4px">${parseInlineMarkdown(line.replace(/^[-*]\s/, ''))}</li>`,
      )
    } else {
      if (inList) {
        inner.push('</ul>')
      ...
      if (line.trim())
        inner.push(
          `<p style="font-size:15px;line-height:1.65;color:#1d1d1f;margin:0 0 4px">${parseInlineMarkdown(line)}</p>`,
        )
    }
  }
  if (inList) inner.push('</ul>')
  return (
    `<div style="border-left:4px solid ${meta.border};background:${meta.bg};border-radius:0 10px 10px 0;padding:14px 18px;margin:24px 0">` +
...
function renderTable(lines: string[]): string {
  ...
  const TH =
    'padding:10px 14px;text-align:left;font-size:14px;font-weight:700;color:#1d1d1f;background:#f5f5f7;border-bottom:2px solid #e0e0e0;white-space:nowrap;letter-spacing:-0.224px'
  const TD =
    'padding:10px 14px;font-size:14px;color:#1d1d1f;border-bottom:1px solid #e8e8e8;line-height:1.6;letter-spacing:-0.224px;vertical-align:middle'

  let html =
    '<div style="overflow-x:auto;margin:0 0 24px;border-radius:10px;border:1px solid #e0e0e0"><table style="width:100%;border-collapse:collapse">'
  html +=
    '<thead><tr>' +
    headers.map((h) => `<th style="${TH}">${parseInlineMarkdown(h)}</th>`).join('') +
    '</tr></thead>'
  html +=
    '<tbody>' +
    dataLines
      .map((row, ri) => {
        ...
        return (
          '<tr>' +
          parseRow(row)
            .map((c) => `<td style="${TD}${bg}">${parseInlineMarkdown(c)}</td>`)
            .join('') +
          '</tr>'
        )
      })
...
function renderMarkdown(md: string): string {
  ...
  while (i < lines.length) {
    ...
    if (ulMatch) {
      if (inOl) closeList()
      if (!inUl) {
        out.push('<ul style="margin:0 0 20px 24px;padding:0;list-style:disc">')
        inUl = true
      }
      out.push(
    ...
    if (olMatch) {
      if (inUl) closeList()
      if (!inOl) {
        out.push('<ol style="margin:0 0 20px 24px;padding:0;list-style:decimal">')
        inOl = true
      }
      out.push(
...
export default async function ContentDetailPage({ params }: PageProps) {
  ...
  const sanitizedHtml = sanitizeHtml(renderMarkdown(post.content), {
