...
export const metadata: Metadata = {
  ...
  description:
    '면허 학원도, 유튜브도 알려주지 않는, 초보운전자가 진짜 막막한 도로 위 모든 순간을 위한 맞춤형 가이드 & AI 챗봇',
      ...
      <body className="min-h-full flex flex-col bg-[var(--canvas)] text-[var(--ink)] antialiased selection:bg-[var(--primary)] selection:text-white">
        ...
        >
          <div className="w-full max-w-[980px] mx-auto px-5 flex items-center justify-between">
            ...
            {/* Desktop nav links */}
            <nav aria-label="주요 메뉴" className="hidden md:flex items-center gap-5">
              {[
                { href: '/', label: '가이드' },
        ...
        >
          <div className="max-w-[980px] mx-auto px-5">
            <div
              className="grid grid-cols-1 md:grid-cols-3 gap-8 pb-10 border-b"
              style={{ borderColor: 'var(--hairline)' }}
            >
              {/* Brand column */}
              <div>
                <p
                  className="text-[14px] font-semibold text-[var(--ink)] mb-3"
            ...
            >
              © {new Date().getFullYear()} DriveTree. 본 정보는 공식 자료 기반의 추정값이며 실제
              처분과 다를 수 있습니다.
            </p>
          </div>
        </footer>
