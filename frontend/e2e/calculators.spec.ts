...
test.describe('계산기 페이지', () => {
  ...
  test.describe('범칙금 계산기', () => {
    test('기본 탭이 범칙금이고 위반 항목 select가 보인다', async ({ page }) => {
      await page.goto('/calculators');
      await expect(page.getByRole('tab', { name: /범칙금/ })).toHaveAttribute('aria-selected', 'true');
  ...
  test.describe('유지비 계산기', () => {
    test('탭 전환 시 유지비 폼이 보인다', async ({ page }) => {
      ...
      await expect(page.getByRole('tab', { name: /유지비/ })).toHaveAttribute('aria-selected', 'true');
